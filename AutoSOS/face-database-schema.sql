...
-- This schema stores face embeddings and user information for facial recognition
...
-- Users can view their own recognition logs
CREATE POLICY "Users can view their own recognition logs" ON public.face_recognition_logs
    FOR SELECT USING (auth.uid()::text = user_id);

-- Service can insert recognition logs (no auth required for service)
...
GRANT SELECT ON public.face_recognition_stats TO authenticated;
...
-- Grant permissions for service (no auth required)
...
-- INSERT INTO public.face_embeddings (user_id, user_name, face_embedding, embedding_dimension) 
-- VALUES ('test_user_1', 'Test User', '\x1234567890abcdef', 128);
