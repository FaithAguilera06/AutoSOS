...
export class RealTimeNavigationPage implements OnInit, OnDestroy {
  ...
  private mechanicLocationInterval: any = null;
  ...
  async ngOnInit() {
    ...
    this.startMechanicLocationTracking();
  }
  ...
  ngOnDestroy() {
    ...
    if (this.mechanicLocationInterval) {
      clearInterval(this.mechanicLocationInterval);
    }
  }
  ...
  addMarkers() {
    ...
    // Add client marker (red)
    new mapboxgl.Marker({ color: '#ff3b30' })
      .setLngLat([this.clientLng, this.clientLat])
      .setPopup(new mapboxgl.Popup().setHTML(`<h4>${this.clientName}</h4><p>Your Location</p>`))
      .addTo(this.map);

    // Add mechanic marker (blue)
    if (this.mechanicLocation) {
      new mapboxgl.Marker({ color: '#0a7cff' })
        .setLngLat([this.mechanicLng, this.mechanicLat])
        .setPopup(new mapboxgl.Popup().setHTML(`<h4>${this.mechanicName}</h4><p>Mechanic Location</p>`))
        .addTo(this.map);
    }
  }
  ...
  async startLocationTracking() {
    try {
      const watchId = await Geolocation.watchPosition(
        ...
        (position) => {
          if (position) {
            this.updateClientLocation(position.coords.latitude, position.coords.longitude);
          }
        }
      );
  ...
  startMechanicLocationTracking() {
    if (!this.mechanicId) return;

    // Fetch mechanic location every 10 seconds
    this.mechanicLocationInterval = setInterval(async () => {
      try {
        ...
        if (!error && mechanicProfile) {
          const newLat = mechanicProfile.latitude;
          const newLng = mechanicProfile.longitude;
          
          // Only update if location has changed significantly
          if (newLat && newLng && 
              (Math.abs(newLat - this.mechanicLat) > 0.0001 || 
               Math.abs(newLng - this.mechanicLng) > 0.0001)) {
            
            this.updateMechanicLocation(newLat, newLng);
            
            // Update mechanic name if available
            if (mechanicProfile.full_name) {
              this.mechanicName = mechanicProfile.full_name;
            }
          }
        }
      } catch (error) {
  ...
  updateClientLocation(lat: number, lng: number) {
    this.clientLocation = { latitude: lat, longitude: lng };
    this.clientLat = lat;
    this.clientLng = lng;

    // Update client marker if map exists
    if (this.map) {
  ...
  updateMechanicLocation(lat: number, lng: number) {
    ...
    // Refresh route if both locations are available
  ...
  callMechanic() {
    if (this.mechanicPhone) {
      window.open(`tel:${this.mechanicPhone}`, '_self');
    } else {
