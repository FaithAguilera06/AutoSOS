...
export class ActivityPage implements OnInit {
  ...
  selectedFilter: 'cancelled' | 'completed' = 'completed';
  ...
  selectedBooking: BookingHistory | null = null;
  ...
  async loadBookings() {
    ...
    try {
      ...
      if (!userId) {
        this.showToast('Please log in to view your booking history', 'warning');
        return;
      }

      // Get bookings with mechanic details
      ...
      if (error) {
        ...
        this.showToast('Error loading booking history', 'danger');
        return;
      }
      ...
      this.bookings = (bookings || []).map(booking => ({
        ...
        mechanicPhone: booking.profiles?.phone || null,
        isProcessing: false
      }));
  ...
  applyFilters() {
    ...
    if (this.searchQuery.trim()) {
      ...
      filtered = filtered.filter(booking => 
        ...
        booking.mechanicName?.toLowerCase().includes(query) ||
   ...
   */
  async payBooking(booking: BookingHistory) {
    this.selectedBooking = booking;
    this.selectedFiles = []; // Reset selected files
    
    // Determine which modal to show based on payment method
    if (booking.payment_method === 'cash') {
      this.isCashModalOpen = true;
    } else if (booking.payment_method === 'facial_recognition') {
      this.isFacialModalOpen = true;
    } else {
  ...
  async confirmCashPayment() {
    ...
    try {
      ...
      await this.bookingService.updateBookingStatus(this.selectedBooking.id, 'completed');
      
      // Update local data
      const bookingIndex = this.bookings.findIndex(b => b.id === this.selectedBooking!.id);
      if (bookingIndex !== -1) {
        this.bookings[bookingIndex].status = 'completed';
        this.bookings[bookingIndex].payment_status = 'paid';
        this.bookings[bookingIndex].payment_method = 'cash';
        this.bookings[bookingIndex].payment_completed_at = new Date().toISOString();
      }
        ...
        if (this.selectedBooking) {
          try {
            ...
            if (bookingIndex !== -1) {
              this.bookings[bookingIndex].status = 'completed';
              this.bookings[bookingIndex].payment_status = 'paid';
              this.bookings[bookingIndex].payment_method = 'facial_recognition';
              this.bookings[bookingIndex].payment_completed_at = new Date().toISOString();
            }
  ...
  navigateToTab(tab: string) {
    switch (tab) {
      case 'home':
        ...
        break;
      case 'activities':
        this.router.navigateByUrl('/client/activity');
        break;
      case 'wallet':
  ...
  async refreshBookings(event: any) {
    try {
      ...
      this.showToast('Error refreshing bookings', 'danger');
    } finally {
   ...
   * Cancel booking
   */
  async cancelBooking(booking: BookingHistory) {
        ...
        },
        {
          ...
          handler: async () => {
            try {
              ...
              this.showToast('Error cancelling booking', 'danger');
            }
          }
        }
      ]
